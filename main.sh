...
download_files() {
    ...
    wget "$CorePure64_url" -O $CorePure64_name
    wget "$CorePure64_url.md5.txt" -O CorePure64.iso.md5.txt
...
extract_file() {
    mount $temp_dir/down/CorePure64-13.1.iso $temp_dir/mnt -o loop,ro
...
echo "tc:Rcdfrd2.718" | chpasswd # Rcdfrd2.718
...
check_tools cpio tar gzip advdef xorriso wget unsquashfs
create_dir
download_files
extract_file
create_iso
rm -fr $temp_dir
_blue "done iso_file: /tmp/TC-remastered.iso\n"
exit 0
